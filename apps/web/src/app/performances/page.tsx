...
import { mockPerformances, mockRehearsals, myBand } from '@/lib/mock-data';
...
type ScheduleTrack = 'performance' | 'rehearsal';

export default function PerformancesPage() {
  const [track, setTrack] = useState<ScheduleTrack>('performance');
  ...
  const sortedRehearsals = [...mockRehearsals].sort(
    (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime(),
  );

  const rehearsalHref = (bandId: string) =>
    bandId === myBand.id ? '/myband' : '/bands';
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h1 className="font-display text-[22px] tracking-[3px] text-muted">일정</h1>
              <span className="flex-1 h-px bg-white/[0.07] min-w-[40px]" />
            </div>
            <p className="text-sm text-muted">공연 일정과 합주 일정을 함께 확인하세요</p>
          </div>
          {track === 'performance' && (
            <Link
        ...
        <div className="flex gap-1 bg-white/[0.04] rounded-[10px] p-[3px] w-fit mb-8">
          <button
            ...
            className={`px-[18px] py-[7px] rounded-lg text-[13px] font-medium transition-all ${
              track === 'performance' ? 'bg-accent text-surface font-bold' : 'text-muted hover:text-stone-50'
            }`}
          >
            공연
          </button>
          <button
            type="button"
            onClick={() => setTrack('rehearsal')}
            className={`px-[18px] py-[7px] rounded-lg text-[13px] font-medium transition-all ${
              track === 'rehearsal' ? 'bg-accent text-surface font-bold' : 'text-muted hover:text-stone-50'
            }`}
          >
            합주
          </button>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 p-5">
                      ...
                      <p className="mt-3 text-sm font-bold text-accent">{formatPrice(performance.ticketPrice)}</p>
                    </div>
                  </Link>
                );
              })}
        ...
        {track === 'rehearsal' && (
          <div className="space-y-4">
            <p className="text-sm text-muted mb-2">
              연습·합주 일정입니다. 내 밴드는 마이 밴드 대시보드로, 다른 밴드는 밴드 탐색으로 이동합니다.
            </p>
            {sortedRehearsals.map((item) => {
              ...
              return (
                <Link
                  key={item.id}
                  href={rehearsalHref(item.bandId)}
                  className="flex bg-surface-card border border-white/[0.07] rounded-[14px] hover:border-white/[0.15] transition-all overflow-hidden group"
                >
                  <div className="flex-shrink-0 w-24 sm:w-28 bg-white/[0.06] border-r border-white/[0.08] flex flex-col items-center justify-center p-4">
                    <span className="text-sm text-accent font-mono-space font-medium">{month}</span>
                    <span className="text-3xl font-display text-stone-50">{day}</span>
                    <span className="text-sm text-muted font-mono-space">{weekday}</span>
                  </div>
                  <div className="flex-1 p-5">
                    <span className="text-[10px] font-mono-space tracking-wider text-accent font-bold uppercase">
                      합주
                    </span>
                    <h3 className="text-lg font-bold text-stone-50 group-hover:text-accent transition-colors mt-1">
                      {item.title}
                    </h3>
                    <p className="mt-1 text-sm text-muted">
                      🕐 {time} · 📍 {item.place}
                    ...
                    <p className="mt-2 text-sm text-subtle">
                      ...
                      <span className="font-bold text-stone-50">
                        {item.bandName}
                        {item.bandId === myBand.id ? (
                          <span className="ml-2 text-[10px] px-2 py-0.5 rounded bg-accent/15 text-accent border border-accent/25">
                            내 밴드
                          </span>
                        ) : null}
                      </span>
