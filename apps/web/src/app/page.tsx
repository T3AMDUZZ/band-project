        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-28 relative">
          <div className="text-center animate-fade-up">
            ...
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              <Link
                href="/bands/new"
                className="w-full sm:w-auto px-8 py-3 border border-accent/40 text-accent font-semibold rounded-lg hover:bg-accent/10 transition-colors text-center"
              >
                밴드 등록하기
              </Link>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {mockPerformances.map((performance, i) => (
              <Link
                ...
                className="bg-surface-card border border-white/[0.07] rounded-[14px] hover:border-white/[0.15] transition-all overflow-hidden group"
              >
                {/* Poster Placeholder */}
                ...
                <div className="p-5">
                  <p className="text-sm text-accent font-mono-space font-medium">
                    {formatDate(performance.date)}
                  </p>
                  <h3 className="mt-1 text-lg font-bold text-stone-50 group-hover:text-accent transition-colors">
                    {performance.title}
                  </h3>
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {performance.bands.map((band, j) => (
                      <span
                        key={band.id}
                        className="inline-block px-2 py-0.5 text-[10px] font-mono-space font-medium tracking-wider uppercase rounded"
                        style={{
                          background: BAND_COLORS[j % BAND_COLORS.length] + '18',
                          color: BAND_COLORS[j % BAND_COLORS.length],
                        }}
                      >
                        {band.name}
                      </span>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex flex-wrap gap-1.5">
                    {band.genre.map((g) => (
                      <span
                        key={g}
                        className="inline-block px-2 py-0.5 text-[10px] font-mono-space tracking-wider bg-white/[0.04] text-muted rounded"
                      >
                        {g}
                      </span>
                    ))}
                  </div>
                  <span className="text-[10px] font-mono-space text-muted bg-white/[0.04] px-2 py-0.5 rounded">
                    👤 {band.memberCount}명
                  </span>
                </div>
              </Link>
            ...
            {mockVenues.map((venue) => (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-base font-bold text-stone-50 group-hover:text-accent transition-colors">
                      {venue.name}
                    </h3>
                    <p className="text-sm text-muted mt-0.5">{venue.address}</p>
                  </div>
                </div>
                <div className="mt-4 flex items-center gap-4 text-sm text-subtle">
                  <span className="flex items-center gap-1 font-mono-space text-xs">
                    👥 수용 {venue.capacity}명
                  </span>
                  <span className="flex items-center gap-1 font-mono-space text-xs">
                    💰 {venue.rentalFee}
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-surface-card border border-white/[0.07] rounded-2xl p-8 sm:p-12 text-center relative overflow-hidden">
            ...
            <h2 className="font-display text-3xl sm:text-4xl tracking-[3px] text-stone-50">
              우리 동아리도 함께하고 싶다면?
            </h2>
            <p className="mt-3 text-subtle max-w-xl mx-auto">
              조직을 등록하고 밴드와 공연을 관리해보세요.
            </p>
            <Link
              href="/organizations/new"
              className="mt-6 inline-block px-8 py-3 bg-accent text-surface font-bold rounded-lg hover:bg-accent-hover transition-colors"
            >
              조직 등록하기
            </Link>
          </div>
